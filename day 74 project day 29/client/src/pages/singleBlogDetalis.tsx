...
const SingleBlogDetails: React.FC = () => {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`http://localhost:3000/getblogs/comment/${id}`, newComment);
  ...
  const handleLike = async () => {
    if (liked || !id) return;
    try {
      await axios.post(`http://localhost:3000/getblogs/like/${id}`);
      setLiked(true);
  ...
  const handleShare = async () => {
    try {
      await axios.post(`http://localhost:3000/getblogs/share/${id}`);
      if (navigator.share) {
