...
export const getProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    const user = (req as any).user; // Auth middleware should set this
    ...
    // Fetch blogs by this user, populate comments' authors
    const blogs = await Blog.find({ author: dbUser._id })
      .sort({ createdAt: -1 })
