...
const SingleBlogDetails: React.FC = () => {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/api/getblogs/comment/${id}`, newComment);
  ...
  const handleLike = async () => {
    if (liked || !id) return;
    try {
      await axios.put(`http://localhost:3000/api/getblogs/like/${id}`);
      setLiked(true);
  ...
  const handleShare = async () => {
    try {
      await axios.put(`http://localhost:3000/api/getblogs/share/${id}`);
      if (navigator.share) {
